 ...
 */
  ...
  public static void setFilesDetails(String mainD){
      ...
      dictFileList[14] = dictFileList[0]+ "dictionaryCom.txt"; //dictFileName
      dictFileList[15] = dictFileList[0]+ "dictionaryCom1.txt"; //dictFileName 
      dictFileList[16] = dictFileList[0]+ "dictionaryCom2.txt"; //dictFileName 
      dictFileList[17] = dictFileList[0]+ "dictionaryCom3.txt"; //dictFileName 
  ...
  public static String cleanWord(String data, boolean stemF )
  {
      ...
      cleaner = cleaner.replaceAll("\\s+", "");
      cleaner = cleaner.toLowerCase();
      
      if (stemF == true && cleaner.length() > 5){
        return cleaner.substring(0, 6);
                ...
                {   
                    ...
                    if (validateWord(data)){
                       ...
                       if (! wordExists(javaWordList,dataValue) ){
                             ...
                             newWord.addDocMapStruct(docData[1],count);
                             newWord.addDocListStruct(docData[1],count);
                          ...
                          (existingWord.getCollectionFrequency()+1);
                           ...
                           existingWord.updateDocMapStruct(docData[1],count);
                           existingWord.updateDocListStruct(docData[1],count);
         ...
         for(JavaParser data : word)
          {
           System.out.printf("%-20s %-20d %-20d ",data.getWordName(),
                  data.getCollectionFrequency(),
                  data.getDocumentFrequence());
                HashMap<Integer,documentID2> hm = data.getDocMapStruct();
           for (Map.Entry<Integer, documentID2> m : hm.entrySet()) {
  ...
  {    
   ...
   public static void serializedFCount(int dataS){
     ...
     try {
         ...
         serilizationData.add(new documentID1(type,count,0));
       ...
       for(JavaParser data : word)
          {   idf = 0.0;
              javaFinalParser newWord = new javaFinalParser();
                  ...
                  newWord.setSearch(data.getwordName());
                  newWord.setCollectionFrequency
                        (data.getCollectionFrequency()); 
                  newWord.setDocSearch(data.getDocSearch().toString());
   ...
   */
    public static StringBuilder importInvertData2(int position, int length, 
            int dataS)
  {
      String outFile;
      int intCounter = 0;
                  ...
                  ( new FileInputStream(outFile));
                   while ( is.available() > 0  ){
                       if(intCounter == position){
                        ...
                        newData.append(Integer.
                                toString(dictionCode[currentP][0])).append("|");
  ...
  */
          ...
          List<JavaParser>  word2, int x)
  {
          for(JavaParser data1 : word2 ){
             if( wordExists(word1,data1.getwordName()) == true ){
  ...
  */
              ...
              System.out.printf("Yes word are being found %s for "
                     ...
                     wordQ.getDocIDInt(),value[i][1]); 
                       ...
                       newWordD.addDocMapStruct(wordD.getWordName(),
                               value[i][1]);
                       double TFIDF = (double) value[i][1] * 
                               (double) wordD.getInvertDocFrequency();
             ...
             if ( wordF.getDocMapStructTFIDF() != null ){
               HashMap<String,Double> hm = wordF.getDocMapStructTFIDF();
                for (Map.Entry m:hm.entrySet()){
                   ...
                   sumSQRS = sumSQRS  +  (double) (prodW * prodW);
                }
          }
  ...
  public static List<docQueryStruct> buildBagOfDocsI(List<javaFinalParser> word,
      ...
      System.out.printf("The query %d had results %d using %s start %d "
              + " end %d %n ", queryNumData[qQuery][0],  docData.size(),qW2, 
              start, end); 
     wordX.clear();
      double sumSQRS, prodW;
      if ( ! docData.isEmpty() ){
  ...
  public static List<docQueryStruct> buildBags( List<javaFinalParser>  query, 
      ...
      List<docQueryStruct> queryData1 = new ArrayList();
             for(int i = 1; i <= size; i++ ){
             System.out.printf("Yes we were called size := %d ,"
                     + "dataS := %d %n", queryNumData[i][0], dataS);
       
             queryData = buildBag(query,i,dataS);
             displayBagTFIDF2(queryData,i,"Query TFIDF"); 
             queryData1.addAll(queryData);
             queryData.clear();
            }
  ...
  {
      List<docQueryStruct> queryData = new ArrayList();
        printDocID2FWordDetails(query,dataS);       
        for(javaFinalParser wordQ : query){
           int[][] value = importInvertData(wordQ.getPointerFile(),
             wordQ.getDocumentFrequence(),dataS );  
             for(int i = 0; i < wordQ.getDocumentFrequence(); i++) {
                if ( value[i][0] == queryNumData[qNum][0] ){
                if ( ! wordExistsDocID(queryData,qNum,dataS)){
                    if (debugType.equalsIgnoreCase("MAD")){          
                         System.out.printf("Yes word are being found %s for "
                          + "query %d in query %s %n ",wordQ.getWordName(),
                         value[i][0],value[i][1]); 
                    }
                    docQueryStruct newWordQ = new docQueryStruct();
                    newWordQ.setDocIDInt(value[i][0]);
                    newWordQ.setDocID(Integer.toString(value[i][0]));
                    newWordQ.addDocMapStruct(wordQ.getWordName(),value[i][1]);
                    double TFIDF = (double) value[i][1] * 
                                   (double) wordQ.getInvertDocFrequency();
                    newWordQ.addDocMapStructTFIDF(wordQ.getWordName(), TFIDF);
                    newWordQ.setTermCount(1);
                    queryData.add(newWordQ);

               } else {
                    docQueryStruct newWordUD = 
                               docSearch(queryData,qNum,dataS);
                    newWordUD.addDocMapStruct(wordQ.getWordName(),value[i][1]);
                    double TFIDF = (double) value[i][1] * 
                               (double) wordQ.getInvertDocFrequency();
                    newWordUD.addDocMapStructTFIDF(wordQ.getWordName(),TFIDF);
                    newWordUD.setTermCount(newWordUD.getTermCount() + 1);
     ...
     double sumSQRS, prodW;
      ...
      for(docQueryStruct wordF : queryData){
          sumSQRS = 0.0;
             if ( wordF.getDocMapStructTFIDF() != null ){
               HashMap<String,Double> hm = wordF.getDocMapStructTFIDF();
                for (Map.Entry m:hm.entrySet()){
      ...
      Collections.sort(queryData, docQueryStruct.docIDComparator);
   ...
   */
    ...
    public static boolean checkFileExist(int queryFile, String typeC){
      ...
      switch(typeC){
       ...
       System.out.printf("Yes we are looking for this %s %s %n",
              type, typeC);
    ...
    public static int[] postProcessData(String[] args, boolean stemF){
        ...
        if (debugType.equalsIgnoreCase("Run") ||
                ...
                debugType.toLowerCase().contains("debug")){
        ...
        if ( checkFileExist(queryDataS[1],"C") == false ) {
         ...
         if ( checkFileExist(queryDataS[0],"Q") == false ) {
            newQueryData = buildBags(updateQuery1,totalQY,queryDataS[0]);
